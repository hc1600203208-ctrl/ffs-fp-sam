...
cv::Mat ConvertMaskImage(const sensor_msgs::msg::Image::ConstSharedPtr &msg)
{
  const auto cv_ptr = cv_bridge::toCvCopy(msg, msg->encoding);
  ...
  else
  {
    ...
    {
      throw std::runtime_error("Unsupported mask encoding: " + msg->encoding);
    }
...
class FoundationPoseTrackerNode : public rclcpp::Node
{
public:
  FoundationPoseTrackerNode()
      : Node("foundationpose_tracker_node"),
        rgb_sub_(),
        depth_sub_()
  {
    DeclareParameters();
    ...
    RCLCPP_INFO(this->get_logger(),
                "FoundationPose ROS 2 node is ready. Waiting for the first synchronized "
                "RGB/Depth/Mask triplet for initial registration.");
  }

private:
  using ImageMsg    = sensor_msgs::msg::Image;
  using RegisterSyncPolicy =
      message_filters::sync_policies::ApproximateTime<ImageMsg, ImageMsg, ImageMsg>;
  using RegisterSyncer = message_filters::Synchronizer<RegisterSyncPolicy>;
  ...
  void DeclareParameters()
  {
    ...
    this->declare_parameter<std::string>("mask_topic", "/camera/mask/image_raw");
  ...
  void SetupRosInterfaces()
  {
    ...
    mask_sub_.subscribe(this, mask_topic_, rmw_qos_profile_sensor_data);

    register_sync_ = std::make_shared<RegisterSyncer>(
        RegisterSyncPolicy(sync_queue_size_), rgb_sub_, depth_sub_, mask_sub_);
    auto *register_policy = register_sync_->getPolicy();
    // 设置最大允许的时间间隔，超过此间隔的帧将被丢弃不匹配。
    register_policy->setMaxIntervalDuration(rclcpp::Duration::from_seconds(0.025));
    register_sync_->registerCallback(std::bind(&FoundationPoseTrackerNode::InitialImageCallback,
                                               this,
                                               std::placeholders::_1,
                                               std::placeholders::_2,
                                               std::placeholders::_3));
  ...
  void SetupTrackingSync()
  {
    if (track_sync_ != nullptr)
    {
      return;
    }

    register_sync_.reset();
    mask_sub_.unsubscribe();
    ...
    // 设置最大允许的时间间隔，超过此间隔的帧将被丢弃不匹配。
    track_policy->setMaxIntervalDuration(rclcpp::Duration::from_seconds(0.025));
    track_sync_->registerCallback(std::bind(&FoundationPoseTrackerNode::TrackingImageCallback,
                                            this,
                                            std::placeholders::_1,
                                            std::placeholders::_2));
  }

  void ScheduleTrackingSyncSetup()
  {
    if (tracking_sync_switch_timer_ != nullptr || track_sync_ != nullptr)
    {
      return;
    }

    tracking_sync_switch_timer_ = this->create_wall_timer(
        std::chrono::milliseconds(1),
        [this]()
        {
          SetupTrackingSync();
          RCLCPP_INFO(this->get_logger(),
                      "Tracking synchronizer is now active. Subsequent pose updates use only "
                      "synchronized RGB/Depth messages.");
          tracking_sync_switch_timer_->cancel();
          tracking_sync_switch_timer_.reset();
        });
  }

  void InitialImageCallback(const ImageMsg::ConstSharedPtr &rgb_msg,
                            ...
                            const ImageMsg::ConstSharedPtr &mask_msg)
  {
    ...
    if (register_attempted_ && !has_pose_)
    {
      RCLCPP_WARN_THROTTLE(
          this->get_logger(),
          *this->get_clock(),
          5000,
          "Initial registration already failed. Restart the node so the first synchronized "
          "RGB/Depth/Mask triplet can be used again.");
      return;
    }
    ...
    try
    {
      ...
      cv::Mat mask  = ConvertMaskImage(mask_msg);
      ...
      if (mask.size() != rgb.size())
      {
        ...
        {
          throw std::runtime_error("The incoming mask size does not match the incoming RGB frame "
                                   "size. Enable resize_mask_to_input if you want the node to "
                                   "resize the mask automatically.");
        }
        ...
        RCLCPP_WARN(this->get_logger(),
                    "Resized the incoming mask from %dx%d to %dx%d to match the stream.",
                    original_mask_cols,
                    original_mask_rows,
                    rgb.cols,
                    rgb.rows);
      ...
      if (!register_attempted_)
      {
        register_attempted_ = true;
        if (RunInitialRegistration(rgb_msg->header, rgb, depth, mask))
        {
          ScheduleTrackingSyncSetup();
        }
        return;
      }
    }
  ...
  void TrackingImageCallback(const ImageMsg::ConstSharedPtr &rgb_msg,
                             const ImageMsg::ConstSharedPtr &depth_msg)
  {
    if (!has_pose_)
    {
    ...
    {
      RCLCPP_ERROR_THROTTLE(
          this->get_logger(), *this->get_clock(), 2000, "Tracking callback failed: %s", e.what());
    }
  }
                              ...
                              const cv::Mat               &registration_mask)
  {
    ...
    if (!ok)
    {
      RCLCPP_ERROR(this->get_logger(), "Initial FoundationPose registration failed.");
      return false;
    }
    ...
    RCLCPP_INFO(this->get_logger(),
                "Initial registration succeeded from the first synchronized RGB/Depth/Mask triplet. "
                "The node switched to RGB/Depth tracking mode.");
    return true;
  }
...
private:
  ...
  bool            register_attempted_{false};
  ...
  SubscriberT                                                   mask_sub_;
  std::shared_ptr<RegisterSyncer>                               register_sync_;
  std::shared_ptr<TrackSyncer>                                  track_sync_;
  rclcpp::TimerBase::SharedPtr                                  tracking_sync_switch_timer_;
};
...
int main(int argc, char **argv)
{
  ...
  try
  {
    auto node = std::make_shared<FoundationPoseTrackerNode>();
    rclcpp::spin(node);
  }
  catch (const std::exception &e)
  {
    RCLCPP_FATAL(rclcpp::get_logger("foundationpose_tracker_node"),
